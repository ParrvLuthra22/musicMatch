/**
 * Calculate match score between two users based on music preferences
 */
...
export const getSharedInterests = (user1, user2) => {
  const sharedGenres = (user1.top_genres || []).filter(genre => 
    (user2.top_genres || []).includes(genre)
  );
  const sharedArtists = (user1.top_artists || []).filter(artist => 
    (user2.top_artists || []).includes(artist)
  );
 ...
 */
export const calculateCompatibility = (matchScore) => {
  if (matchScore >= 80) return { level: 'Excellent', emoji: '🔥' };
  if (matchScore >= 60) return { level: 'Great', emoji: '⭐' };
  if (matchScore >= 40) return { level: 'Good', emoji: '👍' };
  if (matchScore >= 20) return { level: 'Fair', emoji: '🎵' };
